...
void OutputFormat::displayResults(const WorkoutResults& results,
    ...
    // 检查是否所有输出选项都已禁用
    if (!config.showWorkoutPerformance && !config.showCalorieConsumption &&
        !config.showFoodEquivalents && !config.showEquivalentActivity) {
        std::println("请在json中配置需要输出的内容");
        return;
    }
    ...
    if (config.showWorkoutPerformance) {
        std::println("\n[ 运动表现 ]");
    ...
    if (config.showCalorieConsumption) {
        std::println("\n[ 热量消耗 ]");
    ...
    if (config.showEquivalentActivity) {
        std::println("\n[ 等效活动 ]");
